...
# 测试数据
excel_data = excel()
orderDTDAmount_data = excel_data.get_list("bonusForAddDistrict")


@ddt.ddt
class test_bonusForAddDistrict(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_01checkDataForTesting(self,data):
        ...
        if data['team_type'] !='':
            ...
            self.mysql.sql_execute("INSERT INTO `spring2017`.`team` (`id`, `name`, `parent_id`, `leader_id`, `type`, `status`, `is_delete`, `created_dt`, `updated_dt`) VALUES ('"+str(int(data['team_id']))+"', 'helenTeam_"+str(int(data['team_id']))+"', '"+str(int(data['team_parent_id']))+"', '"+sales_id+"', '"+str(int(data['team_type']))+"', '0', '0', UNIX_TIMESTAMP('2016-01-01'), UNIX_TIMESTAMP('2016-01-01'));")
            # 删除本组业绩
    ...
    def test_04insertBonus(self,data):
        '''插入增区奖总记录，设计已有增区奖按12个月发放的情况'''
        if data['send_date']!='':
            self.mysql.sql_execute("DELETE FROM bonus_district_promotion where from_before_team_id="+str(int(data['team_id'])))
            date = self.time_handle.get_timeByMonth(-int(data['promoted_dt']))
            sql = "INSERT INTO `spring2017`.`bonus_district_promotion` (`sales_id`, `date`, `from_sales_id`, `from_before_team_id`, `from_before_team_tsc`, `royalty_rate`, `amount`, `times`, `created_dt`) VALUES ('"+str(int(data['breeder_id']))+"', '"+date+"', '"+str(int(data['salesId']))+"', '"+str(int(data['team_id']))+"', '"+str(float(data['bonus_all'])*12)+"', '0.008', '"+str(float(data['bonus_all']))+"', '"+str(int(data['send_date']))+"', UNIX_TIMESTAMP('"+date+"'));"
            self.mysql.sql_execute(sql)
        ...
        if data['bonus_all']!='':
            try:
                if str(data['bonus_all'])!=u'空':
                    bonusAll_database = self.mysql.get_data("SELECT amount from bonus_district_promotion where  sales_id="+str(int(data['breeder_id']))+" and from_before_team_id="+str(int(data['team_id'])))[0][0]
                    self.assertTrue(float(data['bonus_all'])==float(bonusAll_database))
                else:
                    bonusAll_count = self.mysql.get_data("SELECT COUNT(*) from bonus_district_promotion where sales_id="+str(int(data['breeder_id']))+" and from_before_team_id="+str(int(data['team_id'])))[0][0]
                    self.assertTrue(int(bonusAll_count)==0)
            except Exception as e:
                print str(int(data['breeder_id']))+u"：增区奖（总）不正确"
                raise e
        # 检查增部奖每月发放情况
        if data['bonus_month']!='':
            try:
                if str(data['bonus_month'])!=u'空':
                    bonusFromDatabase = self.mysql.get_data("SELECT district_promotion_b from salary_component WHERE DATE_FORMAT(date,'%Y-%m')=DATE_FORMAT(NOW(),'%Y-%m') and sales_id="+sales_id)[0][0]
