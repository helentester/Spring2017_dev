...
# 测试数据
excel_data = excel()
orderDTDAmount_data = excel_data.get_list("promoted_gtSales")


@ddt.ddt
class test_promoted_gtSales(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_04getResult(self):
        ...
        for downSales in result['keepGtSales']:
            ...
            for p in promoted_type:
                try:
                    if int(float(p)) == -1:    # 降级
                        ...
                        self.assertNotIn(sales_id,self.sales_down+self.sales_upSL)
                    elif int(float(p)) == 1:   # 晋升行销
                        self.assertIn(sales_id,self.sales_upMD)
                    elif int(float(p)) == 2:   # 晋升组长
                        self.assertIn(sales_id,self.sales_upSL)
                    else:   # 异常数据(删除or冻结)
                        self.assertNotIn(sales_id,self.sales_down+self.sales_upMD+self.sales_upSL)
