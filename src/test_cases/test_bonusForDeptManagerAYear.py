...
# 测试数据
excel_data = excel()
orderDTDAmount_data = excel_data.get_list("bonusForDeptManagerAYear")


@ddt.ddt
class test_bonusForDeptManagerAYear(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        if data['bonusAYear']!='':
            try:
                if str(data['bonusAYear'])!='空':
                    bonusFromDatabase = self.mysql.get_data("SELECT dept_year_end_b from salary_component WHERE date='2017-12-01' and sales_id="+sales_id)[0][0]
