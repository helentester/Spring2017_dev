...
# 测试数据
excel_data = excel()
orderDTDAmount_data = excel_data.get_list("AllowanceForBreeNewDept")

@ddt.ddt
class test_AllowanceForBreeNewDept(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_01checkTestData(self,data):
        ...
        promoted_dt = promoted_dts[len(promoted_dts)-1]   # 以最后一个晋升时间为准
        promoted_date = self.time_handle.get_timeByMonth(self.time_handle.get_Current_Month()-int(float(promoted_dt)))
        entry_date = self.time_handle.get_timeByMonth(self.time_handle.get_Current_Month()-int(data['entry_date']))
        # ----处理sales表
        ...
        # ------处理team表
        if str(data['team_leader_id'])!='':
            team_id = str(int(data['team_id']))
            if str(data['team_type'])=='2':
                self.mysql.sql_execute("DELETE FROM team where id="+team_id+" or parent_id="+team_id)
            self.mysql.sql_execute("DELETE from team where id="+team_id)
            self.mysql.sql_execute("INSERT INTO `spring2017`.`team` (`id`, `name`, `parent_id`, `leader_id`, `type`, `status`, `is_delete`, `created_dt`, `updated_dt`) VALUES ('"+team_id+"', 'helen_"+str(int(data['team_id']))+"', '"+str(int(data['team_parent_id']))+"', '"+str(int(data['team_leader_id']))+"', '"+str(int(data['team_type']))+"', '0', '0', '0', '0');")
            # --------删除业绩
            # 删除销售组业绩
            self.mysql.sql_execute("DELETE from order_dtd_account where sales_id in(SELECT id from sales where team_id="+team_id+")")
             # 删除部的业绩
            self.mysql.sql_execute("DELETE from order_dtd_account where sales_id in(SELECT id from sales where team_id in(SELECT id from team WHERE parent_id="+team_id+"))")
            # 删除区的业绩
            self.mysql.sql_execute("DELETE from order_dtd_account where sales_id in(SELECT id from sales where team_id in(SELECT id from team WHERE parent_id in(SELECT id from team WHERE parent_id="+team_id+")))")
        # 删除每个账号的业绩
    ...
    def test_04checkResult_allowanceForBreeNewDept(self,data):
        '''检查部育成津贴'''
        if data['dept_promotion_a']!='':
            allowance = float(data['dept_promotion_a'])
            allowanceFromDatabase = self.mysql.get_data("SELECT dept_promotion_a from salary_component where sales_id="+str(int(data['salesId']))+" and DATE_FORMAT(date,'%Y-%m')=DATE_FORMAT(NOW(),'%Y-%m') ")[0][0]
            try:
    ...
    def test_05checkResult_breederLowerRecord(self):
        ...
        record = self.mysql.get_data("SELECT COUNT(*) from breeder_lower_sales where sales_id=43214433 and breeder_id=43214432")[0][0]
