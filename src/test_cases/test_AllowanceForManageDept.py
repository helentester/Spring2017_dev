...
# 测试数据
excel_data = excel()
orderDTDAmount_data = excel_data.get_list("AllowanceForManageDept")

@ddt.ddt
class test_AllowanceForManageDept(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_05checkResult(self,data):
        '''检查直辖部管理津贴'''
        if data['dept_manager_a']!='':
            sales_id = str(int(data['salesId']))
            if data['dept_manager_a']!=u'空':
                try:
                    allowanceFromDatabase = self.mysql.get_data("SELECT dept_manager_a from salary_component where sales_id="+sales_id+" and DATE_FORMAT(date,'%Y-%m')=DATE_FORMAT(NOW(),'%Y-%m')")[0][0]
                    self.assertTrue(float(data['dept_manager_a'])==float(allowanceFromDatabase))
                except Exception as e:
                    print sales_id
                    raise e
            else:
                try:
                    allowanceRecord = self.mysql.get_data("SELECT COUNT(*)from salary_component where sales_id="+sales_id+" and dept_manager_a>0 and DATE_FORMAT(date,'%Y-%m')=DATE_FORMAT(NOW(),'%Y-%m')")[0][0]
