...
# 测试数据
excel_data = excel()
orderDTDAmount_data = excel_data.get_list("AllowanceForManageDistrict")

@ddt.ddt
class test_AllowanceForManageDistrict(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_05checkResult(self,data):
        ...
        if data['allowance']!='':
            ...
            if data['allowance']!=u'空':
                try:
                    allowanceFromDatabase = self.mysql.get_data("SELECT dist_manager_a from salary_component where sales_id="+sales_id+" and DATE_FORMAT(date,'%Y-%m')=DATE_FORMAT(NOW(),'%Y-%m')")[0][0]
